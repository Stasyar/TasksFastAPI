...
@pytest.mark.parametrize("user_id, full_name, email", test_cases.TEST_USERS_GET_MOCK)
async def test_get_user(user_id, full_name, email, mock_uow, user_service):
    mock_user_obj = User(id=user_id, full_name=full_name, email=email)
...
@pytest.mark.parametrize("full_name, email", test_cases.TEST_USERS_CREATE_MOCK)
...
@pytest.mark.parametrize(
    "task_id, title, description, status", test_cases.TEST_TASKS_GET_MOCK
)
async def test_create_task_with_correct_data(
    task_id, title, description, status, task_service, mock_uow
):
    mock_task = Task(
...
@pytest.mark.parametrize(
    "title, description, status, author_id", test_cases.TEST_TASKS_CREATE_MOCK
)
async def test_create_task_with_correct_data(
    title, description, status, author_id, task_service, mock_uow
):
    ...
    fake_task = Task(
        title=title, description=description, status=status, author_id=author_id
    )
