...
class TestUserRouter:
    ...
    @pytest.mark.parametrize(
        "case",
        test_cases.TEST_USER_ROUTE_CREATE_PARAMS
    )
    async def test_create(
        ...
        with case.expected_error:
            ...
            assert prepare_payload(response, ['user_id', 'detail', 'body']) == case.expected_data
    ...
    @pytest.mark.parametrize(
        'case',
        test_cases.TEST_USER_ROUTE_GET_PARAMS
    )
    async def test_get(
        case: RequestTestCase,
        async_client: AsyncClient,
        ...
        with case.expected_error:
            ...
            assert response.status_code == case.expected_status
            assert prepare_payload(response, ['id', 'created_at', 'detail', 'body']) == case.expected_data
